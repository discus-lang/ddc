...
cleanInstrs mm label binds defs acc (ins@(AnnotInstr i annots) : instrs)
  = let next binds' defs' acc' 
    ...
    in case i of
        ...
        ISet v x1              
         -> do  let binds'  = Map.insert v x1 binds
                next binds' defs acc
        ...
        IReturn (Just x1)
         -> do  next binds defs 
                 $ acc |> (reAnnot $ IReturn (Just (sub x1)))
        ...
        IBranchIf x1 l1 l2
         -> do  next binds defs
                 $ acc  |> (reAnnot $ IBranchIf (sub x1) l1 l2)

        ISwitch x1 def alts
         -> do  next binds defs
                 $ acc  |> (reAnnot $ ISwitch   (sub x1) def alts)
        ...
        IConv  v c x1
         -> do  let defs'   = Map.insert v label defs
                next binds defs'
                 $ acc  |> (reAnnot $ IConv v c (sub x1))

        IGet   v x1 os
         -> do  let defs'   = Map.insert v label defs
                next binds defs'
                 $ acc  |> (reAnnot $ IGet v    (sub x1) os)

        ILoad  v x1
         -> do  let defs'   = Map.insert v label defs
                next binds defs'
                 $ acc  |> (reAnnot $ ILoad v   (sub x1))
