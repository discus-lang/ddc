...
-- | Things that can go wrong when converting Disciple Core Brine to C-code.
--
--   If we get any of these then the program doesn't map onto the features
--   of the C-language.
module DDC.Core.Brine.Output.Error
        (Error(..))
where
...
-- | Things that can go wrong when converting Disciple Core Brine to
--   to C source text.
data Error a
