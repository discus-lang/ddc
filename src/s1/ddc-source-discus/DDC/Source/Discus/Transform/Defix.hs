...
module DDC.Source.Discus.Transform.Defix
        ...
        , Defix         (..))
where
import DDC.Source.Discus.Transform.Defix.FixTable
...
defixExps a table args
 = case args of
        ...
        []         -> error "ddc-source-discus.defixExps: no expressions"

        -- If there is only one element then we're done.
        [RTerm x]  -> Right x
        [_]        -> error "ddc-source-discus.defixExps: did not resolve to a term"
         ...
         -> case defixInfix a table args of
                ...
                Right Nothing
                 -> case r1 of
                        ...
                        _        -> error "ddc-source-discus.defixExps: no term"
